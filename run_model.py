...
model = LSTM_Seq(optimiser = RMSprop(lr=0.01, rho=0.9, epsilon=1e-08, decay=0.0),
                 loss = lambda x , y : custom_mean_squared_error_traces(x,y,margin=10) )

# Load the data
loader = DataLoader('/data/datasets/CTA/ToyNN/test_kde_large_withsig.hdf5')
loader.load(n_training=20000, n_testing=2000, train_start=0)

# Fit the model
model.train(loader.data, nb_epoch=120, batch_size=100, verbose=2,shuffle='batch')
# Save the model
