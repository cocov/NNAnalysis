...
from keras.layers import Embedding,Input,Reshape,Flatten,Permute,Dense
...
N_MAX_PHOTONS_IN_WINDOW = loader.data['train']['label_1'].data.shape[1]*2
...
print(loader.data['train']['traces'][0].shape)
print(loader.data['test']['traces'][0].shape)
...
# Model definition
trace = Input(shape=(N_INPUT_BINS,1), name='input_trace')
# Bidirectional LSTM
encoder = LSTM( units=HIDDEN_LAYER, return_sequences=True)(trace)
encoder = Dropout(0.2)(encoder)
encoder = Dense(10, activation='relu')(encoder)
# Decoder
decoder = RepeatVector(N_INPUT_BINS)(encoder)
decoder = Dense(10, activation='relu')(decoder)
decoder = Dense(HIDDEN_LAYER, activation='relu')(decoder)
decoder = Dropout(0.2)(decoder)
decoder = LSTM( units=1, return_sequences=True)(decoder)

model = Model(inputs=trace, outputs=decoder) #, timing
...
early_stop = EarlyStopping(monitor='loss', min_delta=0.001, patience=10, verbose=2, mode='min')
print(loader.data['train']['traces'].shape,loader.data['test']['traces'].shape)
history = model.fit(loader.data['train']['traces'], np.copy(loader.data['train']['traces']),
          validation_data=[loader.data['test']['traces'], np.copy(loader.data['test']['traces'])],
          epochs=30, batch_size=32, shuffle='batch',verbose =2, callbacks=[early_stop])
